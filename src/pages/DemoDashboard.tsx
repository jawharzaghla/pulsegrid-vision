...
import { getDemoProjectById } from '@/config/demo-projects';
...
interface DemoDashboardProps { tier: DemoTier; }

const DemoDashboard = ({ tier }: DemoDashboardProps) => {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (id) {
      const proj = getDemoProjectById(id);
      if (proj) {
  ...
  }, [localProject?.id, localProject?.widgets.length]);

  useEffect(() => {
    if (localProject && loading) {
      loadData();
    }
  }, [localProject, loadData, loading]);

  const handleRefresh = async () => {
    if (!localProject || refreshing) return;
    setRefreshing(true);
    try {
      if (localProject.widgets.length > 0) {
        const data = await fetchAllDemoWidgetData(localProject.widgets);
  ...
  const onLayoutChange = (layout: Layout) => {
    ...
    setLocalProject(prev => prev ? { ...prev, layout: layoutItems } : null);
  };
  ...
  const exportToPDF = async () => {
    if (!containerRef.current || !localProject) return;
    setIsExporting(true);
    try {
    ...
    <div className="p-4 md:p-8 relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          <Link to={`/demo/${tier}`} className="text-muted-foreground hover:text-foreground transition-colors"><ArrowLeft size={20} /></Link>
          <div className="flex items-center gap-2">
            <span className="text-xl">{localProject!.emoji}</span>
            <h1 className="text-xl font-bold truncate">{localProject!.name}</h1>
          </div>
      ...
      {showDrawer && (
        <AddWidgetDrawer projectId={id || "demo"} editingWidget={editingWidget} isDemo={true} onDemoSave={handleDemoSave} onClose={() => { setShowDrawer(false); setEditingWidget(null); }} onWidgetAdded={() => { setShowDrawer(false); setEditingWidget(null); }} />
      )}
      {showAI && <AIPanel projectId={id || "demo"} widgetPayloads={allPayloads} onClose={() => setShowAI(false)} />}
    </div>
  );
};
